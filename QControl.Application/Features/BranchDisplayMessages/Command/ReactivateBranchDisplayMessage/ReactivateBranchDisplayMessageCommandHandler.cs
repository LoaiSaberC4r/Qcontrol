...
internal sealed class ReactivateBranchDisplayMessageCommandHandler
    : ICommandHandler<ReactivateBranchDisplayMessageCommand, BranchDisplayMessageStateResponse>
{
    ...
    public ReactivateBranchDisplayMessageCommandHandler(
    ...
    public async Task<Result<BranchDisplayMessageStateResponse>> Handle(
        ReactivateBranchDisplayMessageCommand request,
        CancellationToken cancellationToken)
    {
        if (!_currentUser.IsAuthenticated || !_currentUser.UserId.HasValue)
        {
            return Failure("BranchDisplayMessage.Reactivate.Unauthenticated", ErrorMessage.Branch_Authentication_Required, ErrorType.Unauthorized);
        }
        ...
        if (message.IsActive)
        {
            return Failure("BranchDisplayMessage.AlreadyActive", BranchDisplayFeatureMessages.MessageAlreadyActive, ErrorType.Conflict);
        }
        ...
        message.Reactivate(_clock.UtcNow, _currentUser.UserId.Value);
        ...
        return Result<BranchDisplayMessageStateResponse>.Ok(
            BranchDisplayMessageResponseFactory.StateFromEntity(message, BranchDisplayFeatureMessages.MessageReactivated));
    }
