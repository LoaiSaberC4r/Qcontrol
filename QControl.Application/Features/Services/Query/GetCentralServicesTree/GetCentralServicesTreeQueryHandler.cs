...
internal sealed class GetCentralServicesTreeQueryHandler
    : IQueryHandler<
        GetCentralServicesTreeQuery,
        IReadOnlyList<ServiceTreeNodeResponse>>
{
    ...
    public GetCentralServicesTreeQueryHandler(
    ...
    public async Task<Result<IReadOnlyList<ServiceTreeNodeResponse>>> Handle(
        GetCentralServicesTreeQuery request,
        CancellationToken cancellationToken)
    {
        if (!_currentUser.IsAuthenticated || !_currentUser.UserId.HasValue)
        {
            return Result<IReadOnlyList<ServiceTreeNodeResponse>>.Fail(
                new Error(
                    "Services.CentralTree.Unauthenticated",
            ...
            !_currentBranchContext.IsSystemLevelActor)
        {
            return Result<IReadOnlyList<ServiceTreeNodeResponse>>.Fail(
                new Error(
                    "Services.CentralTree.IncludeDeletedForbidden",
        ...
        var globalItems = allItems
            .Where(x => x.Scope == ServiceScope.Global)
            .ToList();
        var states = ServiceHierarchyCalculator.ComputeStates(globalItems);
        var accessContext = await BuildAccessContextAsync(cancellationToken);

        var includedItems = globalItems
