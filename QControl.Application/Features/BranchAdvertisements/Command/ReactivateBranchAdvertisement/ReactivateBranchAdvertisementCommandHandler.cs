...
internal sealed class ReactivateBranchAdvertisementCommandHandler
    : ICommandHandler<ReactivateBranchAdvertisementCommand, BranchAdvertisementStateResponse>
{
    ...
    public ReactivateBranchAdvertisementCommandHandler(
    ...
    public async Task<Result<BranchAdvertisementStateResponse>> Handle(
        ReactivateBranchAdvertisementCommand request,
        CancellationToken cancellationToken)
    {
        ...
        if (advertisement.IsActive)
        {
            return Result<BranchAdvertisementStateResponse>.Fail(new Error(
                "BranchAdvertisements.AlreadyActive",
                BranchFeatureMessages.AlreadyActive,
                ErrorType.Conflict));
        }

        _concurrencyTokenManager.SetOriginalRowVersion(advertisement, rowVersion);

        advertisement.Reactivate(
        ...
        return Result<BranchAdvertisementStateResponse>.Ok(
            BranchAdvertisementResponseFactory.StateFromEntity(
                advertisement,
                BranchFeatureMessages.AdvertisementReactivated));
    }
        ...
        CancellationToken cancellationToken)
    {
        if (!_currentUser.IsAuthenticated || !_currentUser.UserId.HasValue)
        {
            return Result<BranchAdvertisement>.Fail(new Error(
                "BranchAdvertisements.Reactivate.Unauthenticated",
